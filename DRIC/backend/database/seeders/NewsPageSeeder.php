...
class NewsPageSeeder extends Seeder
{
    ...
    private function seedLegacyNews(Section $section, $languages): void
    {
        foreach ($this->legacyNews() as $index => $item) {
            ...
            $block->fill([
                ...
                ], $this->detailData(Str::after($item['href'], '/noticias/'))),
            ]);
            ...
            foreach (['es', 'en'] as $locale) {
                ContentBlockTranslation::updateOrCreate(
                    ...
                    [
                        ...
                        'body' => $this->detailBody(Str::after($item['href'], '/noticias/'), $locale),
    ...
    private function detailData(string $slug): array
    {
        $article = NewsStaticContent::article($slug);
        ...
        return [
            'detail_title_es' => data_get($article, 'heading.es', ''),
            'detail_title_en' => data_get($article, 'heading.en', data_get($article, 'heading.es', '')),
            'deck_es' => data_get($article, 'deck.es', ''),
            'deck_en' => data_get($article, 'deck.en', data_get($article, 'deck.es', '')),
        ];
    }

    private function detailBody(string $slug, string $locale): ?string
    {
        $article = NewsStaticContent::article($slug);
